...
def RaInDeg(Ra):
    import types
 #   if type(Ra)==str or type(Ra)==numpy.str_: #types.StringType:
    if type(Ra)==float:
        return(Ra)
    Ra=str(Ra)
...
def RaInDeg(RA):
    ...
    if isinstance(RA,str) and s.search(RA):
        A = astropy.coordinates.Angle(RA,astropy.units.hour)
    else:
        A = astropy.coordinates.Angle(RA,astropy.units.degree)
    return(A.degree)
       
def DecInDeg(Dec):
    A = astropy.coordinates.Angle(Dec,astropy.units.degree)
    return(A.degree)
